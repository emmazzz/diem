...
pub static MULTI_AGENT_SWAP_SCRIPT: Lazy<Vec<u8>> = Lazy::new(|| {
    ...
    main(alice: &signer, bob: &signer, amount_xus: u64, amount_xdx: u64) {
        ...
        alice_withdrawal_cap = DiemAccount.extract_withdraw_capability(copy(alice));
        bob_addr = Signer.address_of(copy(bob));
        ...
        bob_withdrawal_cap = DiemAccount.extract_withdraw_capability(copy(bob));
        alice_addr = Signer.address_of(copy(alice));
...
pub static MULTI_AGENT_MINT_SCRIPT: Lazy<Vec<u8>> = Lazy::new(|| {
    ...
    main<CoinType>(
        ...
        dd_address = Signer.address_of(copy(dd_account));
        // First, TC mints to DD.
        DiemAccount.tiered_mint<CoinType>(
            move(tc_account), move(dd_address), copy(amount), move(tier_index)
        );

        // Then, DD distributes funds to VASP.
        dd_withdrawal_cap = DiemAccount.extract_withdraw_capability(move(dd_account));
        vasp_address = Signer.address_of(copy(vasp_account));
