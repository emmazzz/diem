...
impl RawTransaction {
    ...
    pub fn sign_multi_agent(
        ...
        let sender_authenticator =
            AccountAuthenticator::ed25519(Ed25519PublicKey::from(sender_private_key), sender_signature);

        let num_secondary_signers = secondary_signers.len();
        if secondary_private_keys.len() != num_secondary_signers {
            return Err(format_err!(
                "number of secondary private keys and number of secondary signers don't match"
            ));
        }
        let mut secondary_authenticators = vec![];
        for i in 0..num_secondary_signers {
            let priv_key = secondary_private_keys[i];
    ...
    pub fn sign_presigned_multi_agent(
        self,
        sender_private_key: &Ed25519PrivateKey,
        ...
        let txn_public_key = Ed25519PublicKey::from(txn_private_key);
        let sender_message = RawTransactionWithData::new_presigned_multi_agent_for_sender (
            self.clone(), txn_public_key.to_bytes().to_vec());
        let sender_signature = sender_private_key.sign(&sender_message);
        let sender_authenticator =
            AccountAuthenticator::ed25519(Ed25519PublicKey::from(sender_private_key), sender_signature);

        let num_secondary_signers = secondary_signer_addresses.len();
        if secondary_private_keys.len() != num_secondary_signers {
            return Err(format_err!(
                "number of secondary private keys and number of secondary signers don't match"
            ));
        ...
        let secondary_signer_message = RawTransactionWithData::new_presigned_multi_agent_for_secondary_signer(
            self.clone(), secondary_signer_addresses.clone(), txn_public_key.to_bytes().to_vec(),
        );
        let mut secondary_authenticators = Vec::new();
        for i in 0..num_secondary_signers {
            let priv_key = secondary_private_keys[i];
            let signature = priv_key.sign(&secondary_signer_message);
            secondary_authenticators.push(AccountAuthenticator::ed25519(
                Ed25519PublicKey::from(priv_key),
                signature,
            ));
        ...
        let txn_auth = AccountAuthenticator::ed25519(
            Ed25519PublicKey::from(txn_private_key),
    ...
    pub fn multi_sign_for_testing(
        ...
        Ok(SignatureCheckedTransaction(SignedTransaction::new(
            self,
            AccountAuthenticator::multi_ed25519(public_key.into(), signature.into()),
        )))
    }
...
pub enum RawTransactionWithData {
    ...
    PresignedMultiAgentForSender {
        raw_txn: RawTransaction,
        txn_pub_key_bytes: Vec<u8>,
    },
    PresignedMultiAgentForSecondarySigner {
        raw_txn: RawTransaction,
        secondary_signer_addresses: Vec<AccountAddress>,
        txn_pub_key_bytes: Vec<u8>,
    }
...
impl RawTransactionWithData {
    ...
    pub fn new_presigned_multi_agent_for_sender(
        raw_txn: RawTransaction,
        txn_pub_key_bytes: Vec<u8>,
    ) -> Self {
        Self::PresignedMultiAgentForSender {
            raw_txn,
            txn_pub_key_bytes,
        }
    }

    pub fn new_presigned_multi_agent_for_secondary_signer(
        raw_txn: RawTransaction,
        secondary_signer_addresses: Vec<AccountAddress>,
        txn_pub_key_bytes: Vec<u8>,
    ) -> Self {
        Self::PresignedMultiAgentForSecondarySigner {
            raw_txn,
            secondary_signer_addresses,
            txn_pub_key_bytes,
        }
...
impl SignedTransaction {
    pub fn new(raw_txn: RawTransaction, sender: AccountAuthenticator) -> Self {
        SignedTransaction {
            raw_txn,
            authenticator: TransactionAuthenticator::new_single_agent(sender),
        }
    }
    ...
    pub fn new_multi_agent(
        ...
        SignedTransaction {
            raw_txn,
            authenticator: TransactionAuthenticator::new_multi_agent(sender,secondary_signer_addresses, secondary_signers)
        }
    }
    ...
    pub fn new_presigned_multi_agent(
        ...
        SignedTransaction {
            raw_txn,
            authenticator: TransactionAuthenticator::new_presigned_multi_agent(sender,secondary_signer_addresses, secondary_signers, txn_auth)
        }
    }
    ...
    pub fn new_multisig(
        ...
        let account_authenticator = AccountAuthenticator::multi_ed25519(public_key, signature);
        SignedTransaction {
            raw_txn,
            authenticator: TransactionAuthenticator::new_single_agent(account_authenticator),
        }
    }
